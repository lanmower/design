...
- 2026-07-06: screen-real-estate + maturity sweep — added `--stage-wide` token decoupling `.app-stage`'s cap from Hero's `--measure-wide`, `.ds-panel-trio`/`.ds-panel-grid` wide-viewport density utilities, migrated all raw `border-radius` literals onto an extended `--r-hair`/`--r-0`/`--r-*` scale (now a build gate alongside the color-token lint); removed all custom web fonts (Bricolage Grotesque/Hanken Grotesk/JetBrains Mono `@import`+`@font-face`) in favor of the system-font fallback chains that were already present. Font-choice rationale (why those specific web fonts were picked pre-removal) drained to rs-learn — query there for the historical AI-tell-font research if needed.
