...
Never add `puppeteer`/`puppeteer-core`/`playwright`/`playwright-core` as a dependency, and never hand-roll a raw headless-Chrome launch in a script. `scripts/a11y-audit.mjs` (Playwright-driven axe-core WCAG check, CI-blocking via ci.yml) and `scripts/visual-baseline.mjs` (Playwright-driven pixel-diff regression) were both removed for this reason — the CI-blocking a11y enforcement they provided has no current replacement, tracked as a residual gap rather than silently dropped. Live browser verification for this kit is done via the gm skill's `browser` verb (direct CDP, no relay), never a repo-owned browser-automation dependency.
